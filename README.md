...
make catalog        # (可选)解析 open-api-front 的 Doc/*.vue → catalog/catalog.json
...
- 车场经验(AI agent 自动沉淀/回忆):存 `~/.config/openydt-cli/park-notes/{parkCode}.{env}.md`(例如 `PTD2YBBZ.test.md`),一车场一环境一文件,物理隔离 test/dev/prod。agent 操作成功后自动追加该车场的有效调用/陷阱/常用车牌,下次自动回忆。清理:删对应 `{parkCode}.{env}.md`。prod 不记真实车牌。
