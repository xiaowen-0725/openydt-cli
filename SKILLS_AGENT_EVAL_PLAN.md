...
Run:
```bash
node --check /Users/zhoujw/develop/tmp/openydt-cli/tools/eval/skills-agent-eval.workflow.mjs && echo "SYNTAX OK"
```
Expected: `SYNTAX OK`(`.mjs` 按 ESM 解析,允许顶层 `export`/`await`;`agent/parallel/phase` 等是 Workflow 运行时注入的全局,`--check` 只验语法不报未定义)。若报语法错,按提示修正再重跑。
