...
func cmdPark_getDisplayVoiceByCarCode(f *cmdutil.Factory) *cobra.Command {
	...
	c.Flags().StringVar(cmdutil.SP(fields, "vipType"), "vip-type", "", "Integer 必填: vip类型(1临时车，2本地vip，5访客车辆，8白名单)")
	c.Flags().StringVar(cmdutil.SP(fields, "featurePassTime"), "feature-pass-time", "", "String: 预计通行时间(yyyyMMddHHmmss)")
	c.Flags().StringVar(cmdutil.SP(fields, "whetherCharge"), "whether-charge", "", "Integer: 是否收费(0：否，1：是)，默认为0")
	return c
}
...
func cmdPark_getParkYdtOtherCarTypeChargeInfo(f *cmdutil.Factory) *cobra.Command {
	...
	c.Flags().StringVar(cmdutil.SP(fields, "startTime"), "start-time", "", "String 必填: 计费开始时间（yyyy-MM-dd HH:mm:ss），用于计算出计费开始时间的1小时，2小时，3小时，4小时，8小时各个时段的费用")
	return c
}
